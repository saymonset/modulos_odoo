...
#
# Replican la lógica de product.template._get_bcv_rate / _get_cop_rate dentro
# de SQL para poder añadir columnas reales a las vistas SQL de los reportes:
#
# bcv_rate_sql (1 USD = X VES):
...
#   4. Fallback 1.0
#
# cop_rate_sql (1 USD = X COP):
#   1. Tasa COP manual de la compañía (res_company.cop_manual_rate)
#   2. currency.rate.provider con provider_type='bogota' y activo (last_rate)
#   3. res.currency.rate para USD con provider Bogotá (original_value)
#   4. Fallback 1.0
...
def cop_rate_sql(company_column):
    """Devuelve una subconsulta escalar con la tasa COP (1 USD = X COP)
    para la compañía indicada por ``company_column`` (ej.: 'po.company_id').
    ...
    return f"""
        COALESCE(
            (SELECT rc.cop_manual_rate FROM res_company rc
              WHERE rc.id = {company_column}
                AND rc.cop_manual_rate_active
                AND rc.cop_manual_rate > 0),
            (SELECT MAX(crp.last_rate) FROM currency_rate_provider crp
              WHERE crp.company_id = {company_column}
                AND crp.provider_type = 'bogota'
                AND crp.active),
            (SELECT r.original_value FROM res_currency_rate r
               JOIN res_currency c ON r.currency_id = c.id
               JOIN currency_rate_provider crp2 ON crp2.id = r.provider_id
