...
@tagged("-at_install", "post_install", "colombia_scraping")
class TestFetchRateBogota(TransactionCase):

    def _create_provider(self):
        return self.env['currency.rate.provider'].create({
            'name': 'Bogota Colombia Test',
            'provider_type': 'bogota',
            'currency_id': self.env.ref('base.COP', raise_if_not_found=False).id
    ...
    def test_01_happy_path(self):
        ...
        html = _load_fixture('colombia_sample.html')
        with patch('requests.get', return_value=self._mock_response(html)):
            rate = provider._fetch_rate_bogota()
        self.assertTrue(rate)
        self.assertAlmostEqual(rate, 3794.91, places=2)
    ...
    def test_03_out_of_bounds_returns_false(self):
        provider = self._create_provider()
        html = "<html><body><h2>1 USD = 500.00 COP</h2></body></html>"
        with patch('requests.get', return_value=self._mock_response(html)):
            rate = provider._fetch_rate_bogota()
        self.assertFalse(rate)
    ...
    def test_05_large_rate_with_comma_thousands(self):
        provider = self._create_provider()
        html = "<html><body><h2>1 USD = 4,123.45 COP</h2></body></html>"
        with patch('requests.get', return_value=self._mock_response(html)):
            rate = provider._fetch_rate_bogota()
        self.assertTrue(rate)
        self.assertAlmostEqual(rate, 4123.45, places=2)